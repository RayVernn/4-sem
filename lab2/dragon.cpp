...
void move(double tau, int Steps) {
    ...
    double const x_0 = 48.5;
double const y_0 = 83;
    if(y > 80) {  // только точки с y > 80 будут вращаться
x = x-x_0;
y = y-y_0;
double omega = 4;
...
public:
    ...
    CalcMesh(const std::vector<double>& nodesCoords, const std::vector<std::size_t>& tetrsPoints) {
        ...
        for(unsigned int i = 0; i < nodesCoords.size() / 3; i++) {
            ...
            double smth = 0;
            if(pointY>80) //на 70 по игрик кончается хвост//на 20 по игрик морда заканчивается
{  nodes[i] = CalcNode(pointX, pointY, pointZ, smth);}
            else
{
 nodes[i] = CalcNode(pointX, pointY, pointZ, smth);
}
    ...
    void snapshot(unsigned int snap_number) {
        ...
        string fileName = "dragon-step-" + std::to_string(snap_number) + ".vtu";
